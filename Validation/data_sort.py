...
with open ('Validation\Validation_load_case.txt') as val_data:
    ...
    with open("sorted_data.txt", "w") as file:
        file.write('ID      x-pos       y-pos      z-pos      vm    |    x-pos       y-pos      z-pos      vm')
        file.write("\n")
        for i, k in sorted_d.items():
            file.write(str(int(i))+'  '+str(k)+'  '+str(y[int(i)-1])+'  '+str(z[int(i)-1])+'  '+str(vm[int(i)-1])+' | ' +str(x2[int(i)-1])+'  '+str(y2[int(i)-1])+'  '+str(z2[int(i)-1])+'  '+str(vm2[int(i)-1]))
            file.write("\n")
